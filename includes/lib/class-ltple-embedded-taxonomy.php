  ...
  /**
	...
	public function register_taxonomy () {
        ...
        $labels = array(
            ...
            'update_item' => sprintf( __( 'Update %s' , LTPLE_EMBEDDED_SLUG ), $this->single ),
            'add_new_item' => sprintf( __( 'Add New %s' , LTPLE_EMBEDDED_SLUG ), $this->single ),
            'new_item_name' => sprintf( __( 'New %s Name' , LTPLE_EMBEDDED_SLUG ), $this->single ),
            'parent_item' => sprintf( __( 'Parent %s' , LTPLE_EMBEDDED_SLUG ), $this->single ),
            'parent_item_colon' => sprintf( __( 'Parent %s:' , LTPLE_EMBEDDED_SLUG ), $this->single ),
            'search_items' =>  sprintf( __( 'Search %s' , LTPLE_EMBEDDED_SLUG ), $this->plural ),
            'popular_items' =>  sprintf( __( 'Popular %s' , LTPLE_EMBEDDED_SLUG ), $this->plural ),
            'separate_items_with_commas' =>  sprintf( __( 'Separate %s with commas' , LTPLE_EMBEDDED_SLUG ), $this->plural ),
            'add_or_remove_items' =>  sprintf( __( 'Add or remove %s' , LTPLE_EMBEDDED_SLUG ), $this->plural ),
            'choose_from_most_used' =>  sprintf( __( 'Choose from the most used %s' , LTPLE_EMBEDDED_SLUG ), $this->plural ),
